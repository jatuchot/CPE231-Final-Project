...
class GradeAssignController extends Controller
{
public function show(){
        return view("grade.gradeimport.step1");
    }
    ...
    public function upload(Request $request){
        ...
        if ($validator->fails()) {
            return view('grade.gradeimport.step1')->withErrors($validator);
        }

        $file = $request->file("file");
        $file = $file->move(storage_path('app/uploads/grade'), Auth::user()->name . '-rawdata.tmp.csv');
        $results = 	Excel::load($file, function($reader){})->toObject();
        $keys = array_keys($results->first()->toArray());
        return view("grade.gradeimport.step2", array('table_name' => $request->input('table_name'), 'keys' => $keys, 'avai_col' => Schema::getColumnListing($request->input("table_name"))));
    ...
    public function insert(Request $request){
        ...
        if ($validator->fails()) {
            return view('grade.gradeimport')->withErrors($validator);
        }
        ...
        $filePath = 'storage/app/uploads/grade/' . Auth::user()->name . '-rawdata.tmp.csv';
        ...
        return redirect('/grade/gradeassign/finish')->with('keys', $finalArray);

    }

    public function finish(){
        if(empty(session("keys"))) return redirect('/grade/gradeassign');
        return view('grade.gradeimport.step3', array('keys' => session("keys")));
    }
